...
| 配置表单         | `SyncConfigDialog`：Release 先验证连接与 identity；Debug 通常只读 |
...
- `configureSync` 是普通配置入口：连接候选 Postgres、读取远端稳定 identity，并与本机绑定核对。不同 identity、来源不可证明的 cursor，或本机与未绑定远端同时已有数据时返回 `Conflict`，不会上传 Outbox、复用旧 cursor 或清空本机。
- 有 cursor 却没有 identity 时进入 `diverged`，阻断普通同步和诊断；不再提供旧绑定采用或旧协议升级入口。
- `SyncConfigDialog` 把 `configureSync` 的 `Conflict` 转成“确认重新绑定”步骤。连接、鉴权、数据库或钥匙串错误保持普通失败，可原位修改和重试；Debug 配置仍只读。
