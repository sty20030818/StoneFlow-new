import type {
	...
	LauncherSessionPhase,
	LauncherSessionState,
} from './sessionTypes'

export function createLauncherSessionState(): LauncherSessionState {
	return {
		closedSessionIds: [],
		phase: { type: 'booting' },
	}
...
export function LauncherSessionReducer(
	state: LauncherSessionState,
	action: LauncherSessionAction,
): LauncherSessionState {
	switch (action.type) {
		case 'frontendBooted':
			return {
...
function matchesActiveSession(phase: LauncherSessionPhase, sessionId: string) {
