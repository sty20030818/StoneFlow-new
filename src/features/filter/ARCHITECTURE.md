...
条件之间是 AND；同一包含条件内的值是 OR，排除条件是该集合的补集。规范化仅稳定排序、条内去重和完全相同条件去重，不能把同字段的独立条件合并成 OR，也不能按日期 token 求交。领域规范化与编码只接受完整 FilterQuery，不把 null/undefined 当作空查询；缺少 URL 参数的处理归路由解码。当前形状必须显式提供 `clauses`；非法字段、操作符或值整体失败。前端与 Rust/SQLite 复用 `tests/fixtures/filter-query-semantics.json` 验证规范化和真实查询成员、计数一致。
...
`ListFilterUiValue.boundary` 只读投影实际查询的 `scope/context/baseViewKey`，名称取现有空间与项目数据；定义未就绪时为 `null`，不伪造默认边界。FilterMenu 在 clean 时也可检查固定边界与完整 `session.effective`，固定边界不能删除；FilterBar 仍只呈现 Draft。所有附加条件按 AND 展示，各条内部多值按 OR。菜单不维护第二份待应用的筛选状态：打开、Escape、点击外部只改变浮层状态；明确选择即时写 URL，关闭不回滚。空态的“调整筛选”复用 `open-menu` 事件，删除条件后焦点回到仍存在的筛选入口或菜单搜索框。
