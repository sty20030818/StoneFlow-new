/**
 * FilterQuery → 现有 list / run_view 查询输入的唯一适配出口。
 * 禁止 UI / scene 内复制映射逻辑。
 */
...
/**
 * run_view 覆盖 filters：直接传 Normalize 后的 FilterQuery（与 View.filters 同形）。
 */
export function adaptFilterQueryToViewFilters(query: FilterQuery): FilterQuery {
