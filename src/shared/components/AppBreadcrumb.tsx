...
export function AppBreadcrumb({ items }: AppBreadcrumbProps) {
	...
	return (
		<Breadcrumbs aria-label='当前位置' className='w-fit min-w-0 max-w-full'>
...
function BreadcrumbNodeItem({
	...
	return (
		<Breadcrumb className={cn('breadcrumbs__item', truncate ? 'min-w-0 shrink' : null)}>
			{showSeparator ? (
				<ChevronRightIcon aria-hidden className='breadcrumbs__separator shrink-0' />
			) : null}
			{isCurrent || !to ? (
				<span
