//! Application ports：UnitOfWork / Outbox / Tombstone 写边界。
//!
//! 这一层的职责只有两件事：
//! - 约束 usecase 可以向外界要求什么能力；
//! - 阻止 Tauri / SeaORM / IPC DTO 继续渗入业务编排层。
//!
//! 写边界由各实体用例持有；Lifecycle 只暴露 Archive / Trash 列表读边界。

pub use crate::activity::ActivityPersistence;
pub use crate::launcher::LauncherPorts;
pub use crate::lifecycle::{LifecycleProjectReader, LifecycleSpaceReader, LifecycleTaskReader};
