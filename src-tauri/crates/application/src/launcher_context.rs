...
mod tests {
	...
	impl FakeLauncherPorts {
		fn ready() -> Self {
			Self {
				...
				candidates: Ok(vec![candidate("space-a", true), candidate("space-b", false)]),
				projects: Vec::new(),
	...
	async fn open_context_should_fail_when_no_space_candidate_is_available() {
		...
		assert!(matches!(error, ApplicationError::DefaultSpaceUnavailable(_)));
	}
	...
	async fn recent_data_should_propagate_storage_errors() {
		...
		assert_eq!(error, ApplicationError::storage("recent tasks query failed"));
	}
}
