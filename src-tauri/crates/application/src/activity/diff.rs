...
pub fn create_changes<T>(
    ...
    new_entity: &T,
    fields: &[&str],
) -> Result<Vec<ActivityChangeInput>, ApplicationError>
where
    T: Serialize,
{
    let old_value = serde_json::to_value(old_entity)
        .map_err(|error| ApplicationError::internal(format!("旧实体序列化失败: {error}")))?;
    let new_value = serde_json::to_value(new_entity)
        .map_err(|error| ApplicationError::internal(format!("新实体序列化失败: {error}")))?;
...
fn serialize_value_for_compare(value: &Option<Value>) -> Result<String, ApplicationError> {
    serde_json::to_string(value)
        .map_err(|error| ApplicationError::internal(format!("Activity 变更值序列化失败: {error}")))
}
