    ...
    SH: LifecycleSyncHook<Connection = SP::Connection>,
{
    ...
    pub async fn archive_project(
        ...
        if current.deleted_at.is_some() {
            return Err(ApplicationError::conflict("已删除 Project 不能归档"));
        }
    ...
    pub async fn restore_task(&self, task_id: &str) -> Result<TaskRecord, ApplicationError> {
        ...
        let target_space = match self.spaces.get(&current.space_id).await? {
            ...
            _ => self
                ...
                .get_default()
                .await?
                .ok_or_else(|| ApplicationError::conflict("默认 Space 不存在"))?,
        };
    ...
    ) -> Result<Vec<crate::lifecycle::types::LifecycleEntry>, ApplicationError> {
    ...
    async fn require_existing_task(&self, task_id: &str) -> Result<TaskRecord, ApplicationError> {
        self.tasks
            .get(task_id)
            .await?
            .ok_or_else(|| ApplicationError::not_found("Task 不存在"))
    ...
    async fn require_visible_space(&self, space_id: &str) -> Result<SpaceRecord, ApplicationError> {
        let space = self
            .spaces
            .get(space_id)
            .await?
...
fn map_lifecycle_guard_error(error: DomainError) -> ApplicationError {
    match error {
        DomainError::Validation(message) => ApplicationError::conflict(message),
    }
}

fn map_space_mutable_error(error: DomainError) -> ApplicationError {
    match error {
        DomainError::Validation(message) => ApplicationError::conflict(message),
    }
}
