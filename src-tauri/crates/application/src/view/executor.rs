...
use crate::{view::types::*, ApplicationError};

use super::service::{TaskViewGroupDto, ViewSortDirection, ViewSortRuleDto};

pub(crate) fn parse_task_view_filters(
    value: &str,
) -> Result<TaskViewFiltersValue, ApplicationError> {
...
pub(crate) fn parse_task_view_filters_value(
    ...
    serde_json::from_value(value.clone())
        .map_err(|error| ApplicationError::validation(format!("Task View filters 非法: {error}")))
}

pub(crate) fn parse_sort_rules(value: &str) -> Result<Vec<ViewSortRuleDto>, ApplicationError> {
    let rules = serde_json::from_str::<Vec<ViewSortRuleDto>>(value)
        .map_err(|error| ApplicationError::validation(format!("Task View sort 非法: {error}")))?;
    normalize_sort_rules(rules)
}

pub(crate) fn parse_group_by(value: Option<&str>) -> Result<TaskGroupBy, ApplicationError> {
    match value {
        ...
        Some(_) => Err(ApplicationError::validation("Task View groupBy 非法")),
    }
}

pub(crate) fn normalize_filters(value: Value) -> Result<Value, ApplicationError> {
    let filters = parse_task_view_filters_value(&value)?;
    serde_json::to_value(filters).map_err(|error| ApplicationError::validation(error.to_string()))
}

pub(crate) fn normalize_sort_rules(
    value: Vec<ViewSortRuleDto>,
) -> Result<Vec<ViewSortRuleDto>, ApplicationError> {
    if value.is_empty() {
        return Err(ApplicationError::validation("View sort 至少需要一条规则"));
    }
...
pub(crate) fn normalize_group_by(
    value: Option<String>,
...
fn parse_json_value(value: &str) -> Result<Value, ApplicationError> {
    serde_json::from_str(value)
        .map_err(|error| ApplicationError::validation(format!("View JSON 非法: {error}")))
}

fn parse_sort_field(value: &str) -> Result<TaskSortField, ApplicationError> {
    match value {
        ...
        _ => Err(ApplicationError::validation("Task View sort 字段非法")),
    }
}
