    ...
    L: ViewLookupReader,
{
    ...
    pub async fn update_view(&self, input: UpdateViewInput) -> Result<ViewDto, ApplicationError> {
        ...
        if current.kind == ViewKind::System {
            return Err(ApplicationError::conflict("系统 View 不允许编辑"));
        }
    ...
    pub async fn delete_view(&self, input: DeleteViewInput) -> Result<(), ApplicationError> {
        let view_id = normalize_required_text(&input.view_id, "View id")?;
        let current = self
            .persistence
            .get(&view_id)
        ...
        if current.kind == ViewKind::System {
            return Err(ApplicationError::conflict("系统 View 不允许删除"));
        }
    ...
    async fn resolve_view(
        ...
        if view.entity_type != entity_type {
            return Err(ApplicationError::validation("View 实体类型不匹配"));
        }
    ...
    ) -> Result<Vec<ViewTaskListItemDto>, ApplicationError> {
        let space_ids = tasks
            .iter()
...
fn normalize_task_scope(
    input: &ViewTaskScopeInput,
) -> Result<NormalizedTaskScope, ApplicationError> {
    match input.kind {
...
) -> Result<ViewTaskPlacementQuery, ApplicationError> {
