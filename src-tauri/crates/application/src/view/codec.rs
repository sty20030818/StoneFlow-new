...
pub fn view_sync_fields(record: &ViewRecord) -> Result<Map<String, Value>, ApplicationError> {
    ...
    Ok(Map::from_iter([
        ...
        ("scope".to_owned(), json!(scope)),
        ("filters".to_owned(), json!(definition)),
        ("sort".to_owned(), json!([])),
...
/// 只物化完整定义；残缺 patch 必须先由协议层补齐，不能在这里填空业务字段。
...
mod tests {
    ...
    fn damaged_or_missing_business_definition_is_never_replaced_with_empty_filters() {
        ...
        for invalid in [
            ...
            assert!(view_record_from_sync_fields("view-1", 3, &fields).is_err());
        }
        for missing in [
            "name",
            "entity_kind",
