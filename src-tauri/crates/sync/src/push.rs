...
pub async fn push_local_changes(local: &Connection, remote: &Connection) -> Result<(), SyncError> {
    loop {
        ...
        let remote_tx = remote.transaction().await.map_err(|error| {
            SyncError::remote_database(format!("开启远端 push 事务失败: {error}"))
        })?;
        ...
        remote_tx.commit().await.map_err(|error| {
            SyncError::remote_database(format!("提交远端 push 事务失败: {error}"))
        })?;
...
fn parse_mutation_payload(record: &LocalMutationRecord) -> Result<SyncOperationPayload, SyncError> {
    let value = serde_json::from_str::<Value>(&record.payload).map_err(|error| {
        SyncError::serialization(format!("解析本地 mutation payload 失败: {error}"))
    })?;
