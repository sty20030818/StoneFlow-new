//! 本地 SQLite 同步元数据读写与 snapshot 清理。
...
pub async fn list_pending_mutations(
    local: &Connection,
    limit: u64,
) -> Result<Vec<LocalMutationRecord>, SyncError> {
    let mut rows = local
        .query(
            r#"
        ...
        .await
        .map_err(|error| SyncError::local_database(format!("读取本地 sync_mutations 失败: {error}")))?;
    let mut records = Vec::new();

    while let Some(row) = rows.next().await.map_err(|error| {
        SyncError::local_database(format!("遍历本地 sync_mutations 失败: {error}"))
    })? {
...
pub async fn mark_mutations_acked(
    ...
    let transaction = local.transaction().await.map_err(|error| {
        SyncError::local_database(format!("开启本地 mutation ack 事务失败: {error}"))
    })?;
    ...
    for record in batch {
        transaction
            ...
            .await
            .map_err(|error| {
                SyncError::local_database(format!("更新本地 sync_mutations 状态失败: {error}"))
            })?;
    }

    transaction.commit().await.map_err(|error| {
        SyncError::local_database(format!("提交本地 mutation ack 事务失败: {error}"))
    })?;
    Ok(())
}

pub async fn read_server_seq_cursor(local: &Connection) -> Result<Option<i64>, SyncError> {
    ...
    raw.parse::<i64>()
        .map(Some)
        .map_err(|error| SyncError::serialization(format!("解析 server_seq cursor 失败: {error}")))
}
...
pub async fn upsert_sync_shadow(
    ...
    transaction
        ...
        .await
        .map_err(|error| SyncError::local_database(format!("写入 sync_shadow 失败: {error}")))?;
    Ok(())
}
...
pub async fn delete_sync_shadow(
    ...
    transaction
        ...
        .await
        .map_err(|error| SyncError::local_database(format!("删除 sync_shadow 失败: {error}")))?;
    Ok(())
}

pub async fn reset_local_replica_for_snapshot(transaction: &Transaction) -> Result<(), SyncError> {
    // snapshot pull 会用远端当前镜像重建空本地副本，因此先清空所有可重建业务表和同步元数据。
    let statements = [
    ...
    for statement in &statements[..6] {
        transaction
            ...
            .await
            .map_err(|error| {
                SyncError::local_database(format!("清空本地 snapshot 副本失败: {error}"))
            })?;
    }
    ...
    transaction
        ...
        .await
        .map_err(|error| {
            SyncError::local_database(format!("清空本地 snapshot settings 失败: {error}"))
        })?;
    transaction
        .execute(statements[7], params![])
        .await
        .map_err(|error| {
            SyncError::local_database(format!("清空本地 snapshot spaces 失败: {error}"))
        })?;
    transaction
        .execute(statements[8], params![DEVICE_ID_SCOPE])
        .await
        .map_err(|error| {
            SyncError::local_database(format!("清空本地 snapshot cursor 失败: {error}"))
        })?;

    Ok(())
}

async fn read_text_cursor(local: &Connection, scope: &str) -> Result<Option<String>, SyncError> {
    let mut rows = local
        .query(
        ...
        .await
        .map_err(|error| SyncError::local_database(format!("读取 sync_cursor 失败: {error}")))?;
    let row = rows
        .next()
        .await
        .map_err(|error| SyncError::local_database(format!("遍历 sync_cursor 失败: {error}")))?;
...
async fn write_text_cursor_in_transaction(
    ...
    transaction
        ...
        .await
        .map_err(|error| SyncError::local_database(format!("写入 sync_cursor 失败: {error}")))?;
    Ok(())
}

fn read_local_column_error(column: &'static str) -> impl FnOnce(libsql::Error) -> SyncError {
    move |error| SyncError::local_database(format!("读取 {column} 失败: {error}"))
}
