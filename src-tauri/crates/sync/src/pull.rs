...
pub async fn pull_remote_changes(local: &Connection, remote: &Connection) -> Result<(), SyncError> {
    ...
    loop {
        ...
        let transaction = local.transaction().await.map_err(|error| {
            SyncError::local_database(format!("开启本地 pull 事务失败: {error}"))
        })?;
        ...
        transaction.commit().await.map_err(|error| {
            SyncError::local_database(format!("提交本地 pull 事务失败: {error}"))
        })?;
    }
...
async fn pull_snapshot(local: &Connection, remote: &Connection) -> Result<(), SyncError> {
    let snapshot = fetch_restore_snapshot(remote).await?;
    let server_seq = fetch_latest_server_seq(remote).await?.unwrap_or(0);
    let transaction = local.transaction().await.map_err(|error| {
        SyncError::local_database(format!("开启本地 snapshot 事务失败: {error}"))
    })?;
    ...
    transaction.commit().await.map_err(|error| {
        SyncError::local_database(format!("提交本地 snapshot 事务失败: {error}"))
    })?;
    Ok(())
}
...
async fn apply_snapshot_payload(
    ...
    let snapshot = serde_json::to_string(&operation.payload).map_err(|error| {
        SyncError::serialization(format!("序列化 sync_shadow snapshot 失败: {error}"))
    })?;
    upsert_sync_shadow(
        transaction,
        entity_type,
