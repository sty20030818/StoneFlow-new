...
async fn apply_operation(
    ...
    match (&operation.action, &operation.payload) {
        ...
        (SyncAction::Upsert, SyncOperationPayload::HardDelete { .. }) => Err(SyncError::protocol(
            "hard delete payload 不允许使用 upsert action",
        )),
    }
}
...
async fn insert_activity_tombstone(
    ...
    let metadata = tombstone
        ...
        .transpose()
        .map_err(|error| {
            SyncError::serialization(format!("序列化 tombstone metadata 失败: {error}"))
        })?;
    transaction
        .execute(
            r#"
...
impl ApplyTarget {
    fn database_error(self, message: String) -> SyncError {
        match self {
            Self::Local => SyncError::local_database(message),
            Self::Remote => SyncError::remote_database(message),
        }
    }
}
...
mod tests {
    ...
    async fn apply_operation_to_local_should_report_local_database_error_when_schema_missing() {
        ...
        assert!(
            matches!(error, SyncError::LocalDatabase { .. }),
    ...
    async fn apply_operation_to_remote_should_report_remote_database_error_when_schema_missing() {
        ...
        assert!(
            matches!(error, SyncError::RemoteDatabase { .. }),
    ...
    async fn apply_operation_to_local_should_return_protocol_error_for_unknown_hard_delete_type() {
        ...
        assert!(
            matches!(error, SyncError::Protocol { .. }),
