...
use crate::{supervisor::SupervisorHandle, MAIN_WINDOW_LABEL};
use desktop_app::{
    app::{
        error::AppError,
...
pub async fn start_ipc_server(
    ...
    helper_state
        .set_ipc_status(desktop_app::app::state::IpcServerStatus::Starting, None)
        .await;
    ...
    helper_state
        .set_ipc_status(desktop_app::app::state::IpcServerStatus::Ready, None)
        .await;
...
pub async fn shutdown(helper_state: CommandHelperState) {
    helper_state
        .set_ipc_status(desktop_app::app::state::IpcServerStatus::Stopped, None)
        .await;
}
    ...
    loop {
        let stream = match listener.accept().await {
            ...
            Err(error) => {
                helper_state
                    .set_ipc_status(
                        desktop_app::app::state::IpcServerStatus::Error,
...
async fn handle_ipc_connection(
    ...
    let service = crate::quick_services::build_quick_create_service(&database);
    let session_bridge = crate::quick_services::build_quick_create_session_bridge(&database);
    ...
    dispatch_command_open(
        ...
        CommandOpenPayload {
            ...
            placement: match target.placement {
                desktop_app::application::services::QuickResolvedPlacement::Project => "project",
                desktop_app::application::services::QuickResolvedPlacement::Inbox => "inbox",
                desktop_app::application::services::QuickResolvedPlacement::NoProject => "no_project",
            },
        },
    )
    .await
            ...
            dispatch_command_open(
                ...
                CommandOpenPayload {
                    ...
                    placement: match target.placement {
                        desktop_app::application::services::QuickResolvedPlacement::Project => "project",
                        desktop_app::application::services::QuickResolvedPlacement::Inbox => "inbox",
                        desktop_app::application::services::QuickResolvedPlacement::NoProject => {
                            "no_project"
                        }
                    },
                },
...
mod tests {
    use super::*;
    use desktop_app::app::state::HelperLifecycleStage;
