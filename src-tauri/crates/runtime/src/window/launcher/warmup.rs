...
pub async fn ensure_launcher_ready(
	...
	if warmup.begin_warming().await {
		...
		tauri::async_runtime::spawn(async move {
			match tokio::time::timeout(FRONTEND_READY_TIMEOUT, timeout_state.wait_until_ready()).await {
				Ok(Ok(())) => {}
				Ok(Err(error)) => log::warn!("launcher.warmup_failed error={error}"),
				Err(_) => {
					log::warn!("launcher.warmup_timed_out timeout_ms={}", FRONTEND_READY_TIMEOUT.as_millis());
					timeout_state
...
mod tests {
	...
	async fn failed_warmup_should_be_retryable() {
		...
		assert_eq!(
			state.wait_until_ready().await.expect_err("failed warmup should surface error"),
			"WebView 创建失败"
		);
		assert!(state.begin_warming().await);
	}
	...
	async fn waiting_for_frontend_should_not_block_failure_transition() {
		...
		assert_eq!(
			waiting.await.expect("waiting task should finish").expect_err("failure should surface"),
			"WebView 启动失败"
		);
	}
}
