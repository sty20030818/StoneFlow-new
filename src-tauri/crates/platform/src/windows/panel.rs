//! Launcher Windows 浮窗生命周期。
//!
//! Windows 没有 macOS `NSPanel` 等价物；这里使用标准 `WebviewWindow`
//! 承载透明固定壳；尺寸与 macOS 共用 `launcher_window::spec`。
...
use crate::launcher_window::{
    spec::{
        LAUNCHER_LABEL, LAUNCHER_PANEL_RADIUS, LAUNCHER_TITLE, LAUNCHER_URL,
        LAUNCHER_WINDOW_HEIGHT, LAUNCHER_WINDOW_WIDTH,
    },
    LauncherWindowCallbacks,
};

/// 在 Tauri `setup()` 阶段预创建 Launcher 浮窗，默认隐藏等待快捷键唤起。
/// Windows 外圆角由 FE `--launcher-panel-radius` 对齐 `LAUNCHER_PANEL_RADIUS`（8）。
pub fn init_launcher_panel(app_handle: &AppHandle<Wry>, callbacks: LauncherWindowCallbacks) {
    ...
    .build()
    {
        Ok(window) => window,
        Err(error) => {
            log::error!("platform: windows launcher 窗口创建失败: {error}");
            return;
        }
    };
...
pub fn present_launcher_window(
    ...
    window
        .show()
        .map_err(|error| format!("显示 windows launcher 失败: {error}"))?;

    if let Err(error) = window.set_focus() {
        log::warn!("platform: 聚焦 windows launcher 失败: {error}");
    }
...
pub fn is_launcher_window_visible(app_handle: &AppHandle<Wry>) -> Result<bool, String> {
    let Some(window) = app_handle.get_webview_window(LAUNCHER_LABEL) else {
        return Err("launcher 窗口未初始化".to_owned());
    };
    ...
    window
        .is_visible()
        .map_err(|error| format!("读取 windows launcher 可见状态失败: {error}"))
}

pub fn hide_launcher_window(app_handle: &AppHandle<Wry>) -> Result<(), String> {
    let Some(window) = app_handle.get_webview_window(LAUNCHER_LABEL) else {
        return Err("launcher 窗口未初始化".to_owned());
    };
    ...
    window
        .hide()
        .map_err(|error| format!("隐藏 windows launcher 失败: {error}"))
}

pub fn prepare_hidden_launcher_window(app_handle: &AppHandle<Wry>) -> Result<(), String> {
    let Some(window) = app_handle.get_webview_window(LAUNCHER_LABEL) else {
        return Err("launcher 窗口未初始化".to_owned());
    };
...
fn reset_webview_zoom(window: &WebviewWindow<Wry>) {
    if let Err(error) = window.set_zoom(1.0) {
        log::warn!("platform: 重置 windows launcher WebView zoom 失败: {error}");
    }
}

/// Windows 标准窗口可以可靠收到 Tauri focus 事件；失焦即隐藏，贴近面板语义。
fn install_focus_auto_hide(window: &WebviewWindow<Wry>, callbacks: LauncherWindowCallbacks) {
    ...
    window.on_window_event(move |event| {
        ...
        match window_for_hide.is_visible() {
            Ok(true) => {
                log::info!("platform: windows launcher 失焦 → hide window");
                if let Err(error) = window_for_hide.hide() {
                    log::warn!("platform: 失焦隐藏 windows launcher 失败: {error}");
                }
                let app_handle = window_for_hide.app_handle().clone();
                on_resign_key(app_handle);
            }
            Ok(false) => {}
            Err(error) => log::warn!("platform: 读取 windows launcher 可见状态失败: {error}"),
        }
    });
}
...
pub(crate) fn position_window_on_active_monitor(window: &WebviewWindow<Wry>) {
    let (monitor, cursor_position) = match active_monitor_from_cursor(window) {
        ...
        None => {
            ...
            if let Err(error) = window.center() {
                log::warn!("platform: windows launcher center 失败: {error}");
            }
            return;
        }
    };
    ...
    if let Err(error) = window.set_size(logical_size) {
        log::warn!("platform: windows launcher 强制规格尺寸失败: {error}");
    }
    let window_width = LAUNCHER_WINDOW_WIDTH * scale_factor;
    let window_height = LAUNCHER_WINDOW_HEIGHT * scale_factor;
    ...
    if let Err(error) = window.set_position(position) {
        log::warn!("platform: 定位 windows launcher 失败: {error}");
        if let Err(error) = window.center() {
            log::warn!("platform: windows launcher center 失败: {error}");
        }
        return;
    ...
    log::info!(
        "platform: windows launcher 居中到鼠标所在屏 cursor=({},{}) work_area=({},{},{}×{}) scale={} window=({}×{}) → origin=({},{})",
        cursor_position
            .as_ref()
