...
impl ViewRepository {
    ...
    pub async fn get_in_connection<C: ConnectionTrait>(
        ...
        Ok(View::find_by_id(view_id).one(connection).await?)
    }
...
mod tests {
    ...
    async fn reads_do_not_mutate_business_record_or_enqueue_outbox() {
        ...
        let before = outbox.count_all().await.unwrap();
        for _ in 0..2 {
            assert_eq!(
                repository.get("view-1").await.unwrap(),
                Some(expected.clone())
            );
            assert_eq!(repository.list().await.unwrap(), vec![expected.clone()]);
        }
        assert_eq!(
            View::find_by_id("view-1").one(connection).await.unwrap(),
            Some(expected)
        );
        assert_eq!(outbox.count_all().await.unwrap(), before);
    }
}
