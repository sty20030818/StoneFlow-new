...
async fn task_link_repository_should_support_crud_roundtrip() {
    ...
    let updated = repository
        .update(
            &transaction,
            "link-1",
            crate::infrastructure::repositories::UpdateTaskLinkPatch {
...
async fn task_link_service_should_record_activity_and_keep_task_detail_unchanged() {
    ...
    let timeline = activity_service
        .get_entity_activities(GetEntityActivitiesInput {
            entity_type: stoneflow_entity::common::ActivityEntityKind::Task,
...
async fn create_task_fixture(
    database: &crate::infrastructure::database::DatabaseRuntimeState,
    title: &str,
) -> crate::application::services::TaskDetailDto {
