...
pub async fn open_local_sqlite(database_path: &str) -> Result<Connection, SyncWorkerError> {
    let database = Builder::new_local(Path::new(database_path))
        .build()
        .await
        .map_err(|error| SyncWorkerError::internal(format!("打开本地 SQLite 失败: {error}")))?;

    database
        .connect()
        .map_err(|error| SyncWorkerError::internal(format!("连接本地 SQLite 失败: {error}")))
}
...
pub async fn open_remote(remote_config: &SyncRemoteConfig) -> Result<Connection, SyncWorkerError> {
    ...
    database
        .connect()
        .map_err(|error| SyncWorkerError::internal(format!("连接 Turso 远端失败: {error}")))
}

pub async fn bootstrap_remote_schema(remote: &Connection) -> Result<(), SyncWorkerError> {
    for statement in REMOTE_SCHEMA_STATEMENTS {
        remote.execute_batch(statement).await.map_err(|error| {
            SyncWorkerError::internal(format!("初始化 Turso 远端表结构失败: {error}"))
        })?;
    }
...
pub async fn insert_operation_if_absent(
    ...
    let payload = serde_json::to_string(&operation.payload)
        .map_err(|error| SyncWorkerError::internal(format!("序列化 sync payload 失败: {error}")))?;
    let changed = transaction
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("写入远端 sync_operations 失败: {error}")))?;
...
pub async fn fetch_operations_after(
    ...
    let mut rows = remote
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("读取远端 sync_operations 失败: {error}")))?;
    let mut operations = Vec::new();

    while let Some(row) = rows
        .next()
        .await
        .map_err(|error| SyncWorkerError::internal(format!("遍历远端 sync_operations 失败: {error}")))?
    {
        let action = match row
            .get::<String>(5)
            .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.action 失败: {error}")))?
            .as_str()
        {
        ...
        let payload_raw = row
            .get::<String>(6)
            .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.payload 失败: {error}")))?;
        let payload = serde_json::from_str::<SyncOperationPayload>(&payload_raw)
            .map_err(|error| SyncWorkerError::internal(format!("解析远端 sync payload 失败: {error}")))?;
        operations.push(RemoteOperationRecord {
            remote_cursor: row
                .get::<i64>(0)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.remote_cursor 失败: {error}")))?,
            op_id: row
                .get::<String>(1)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.op_id 失败: {error}")))?,
            device_id: row
                .get::<String>(2)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.device_id 失败: {error}")))?,
            entity_type: row
                .get::<String>(3)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.entity_type 失败: {error}")))?,
            entity_id: row
                .get::<String>(4)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.entity_id 失败: {error}")))?,
            action,
            payload,
            committed_at: row
                .get::<String>(7)
                .map_err(|error| SyncWorkerError::internal(format!("读取远端 operation.committed_at 失败: {error}")))?,
        });
    }
...
fn map_remote_connect_error(error: libsql::Error) -> SyncWorkerError {
    ...
    if message.contains("401 Unauthorized") || message.contains("invalid JWT token") {
        return SyncWorkerError::validation(
            "Turso 鉴权失败，请确认当前保存的 token 是该数据库对应的 auth token。",
        );
    }

    SyncWorkerError::internal(format!("初始化 Turso 远端连接失败: {message}"))
}
