...
pub async fn apply_operation_to_remote(
    ...
    apply_operation(transaction, operation).await
}
...
async fn apply_operation(
    ...
    match (&operation.action, &operation.payload) {
        ...
        | (SyncAction::Delete, SyncOperationPayload::Space { snapshot }) => {
            upsert_space(transaction, snapshot).await
        }
        (SyncAction::Upsert, SyncOperationPayload::Project { snapshot })
        | (SyncAction::Delete, SyncOperationPayload::Project { snapshot }) => {
            upsert_project(transaction, snapshot).await
        }
        (SyncAction::Upsert, SyncOperationPayload::Task { snapshot })
        | (SyncAction::Delete, SyncOperationPayload::Task { snapshot }) => {
            upsert_task(transaction, snapshot).await
        }
        (SyncAction::Upsert, SyncOperationPayload::View { snapshot }) => {
            upsert_view(transaction, snapshot).await
        }
        (SyncAction::Delete, SyncOperationPayload::View { snapshot }) => {
            delete_view(transaction, snapshot, &operation.op_id).await
        }
        (SyncAction::Upsert, SyncOperationPayload::Setting { snapshot })
        | (SyncAction::Delete, SyncOperationPayload::Setting { snapshot }) => {
            upsert_setting(transaction, snapshot).await
        }
        (SyncAction::Upsert, SyncOperationPayload::TaskLink { snapshot }) => {
            upsert_task_link(transaction, snapshot).await
        }
        (SyncAction::Delete, SyncOperationPayload::TaskLink { snapshot }) => {
            delete_task_link(transaction, snapshot, &operation.op_id).await
        }
        (SyncAction::Delete, SyncOperationPayload::HardDelete { target }) => {
            apply_hard_delete(transaction, target, &operation.op_id).await
        }
        (SyncAction::Upsert, SyncOperationPayload::HardDelete { .. }) => Err(
            SyncWorkerError::internal("hard delete payload 不允许使用 upsert action"),
        ),
    }
}

async fn upsert_space(transaction: &Transaction, snapshot: &SpacePayload) -> Result<(), SyncWorkerError> {
    if !should_apply_record(transaction, "spaces", "id", &snapshot.id, &snapshot.updated_at, None).await? {
        return Ok(());
    }
    ...
    if snapshot.is_default {
        ...
        ).await.map_err(|error| SyncWorkerError::internal(format!("清理远端默认 Space 失败: {error}")))?;
    }
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 Space 镜像失败: {error}")))?;

    Ok(())
}

async fn upsert_project(transaction: &Transaction, snapshot: &ProjectPayload) -> Result<(), SyncWorkerError> {
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 Project 镜像失败: {error}")))?;

    Ok(())
}

async fn upsert_task(transaction: &Transaction, snapshot: &TaskPayload) -> Result<(), SyncWorkerError> {
    if !should_apply_record(
        transaction,
        "tasks",
        "id",
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 Task 镜像失败: {error}")))?;

    Ok(())
}

async fn upsert_view(transaction: &Transaction, snapshot: &ViewPayload) -> Result<(), SyncWorkerError> {
    if !should_apply_record(
        transaction,
        "views",
        "id",
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 View 镜像失败: {error}")))?;

    Ok(())
}

async fn delete_view(transaction: &Transaction, snapshot: &ViewPayload, op_id: &str) -> Result<(), SyncWorkerError> {
    ...
    transaction
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("删除 View 镜像失败: {error}")))?;
...
async fn upsert_setting(transaction: &Transaction, snapshot: &SettingPayload) -> Result<(), SyncWorkerError> {
    if !should_apply_record(transaction, "settings", "key", &snapshot.key, &snapshot.updated_at, None).await? {
        return Ok(());
    }
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 Setting 镜像失败: {error}")))?;

    Ok(())
}

async fn upsert_task_link(transaction: &Transaction, snapshot: &TaskLinkPayload) -> Result<(), SyncWorkerError> {
    if !should_apply_task_link(transaction, &snapshot.id, &snapshot.updated_at).await? {
        return Ok(());
    }
    ...
    ).await.map_err(|error| SyncWorkerError::internal(format!("写入 TaskLink 镜像失败: {error}")))?;
...
async fn delete_task_link(
    ...
    if !should_delete_task_link(transaction, &snapshot.id, &snapshot.updated_at).await? {
        return Ok(());
    }
    ...
    transaction
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("删除 TaskLink 镜像失败: {error}")))?;
    insert_activity_tombstone(
        transaction,
        op_id,
        "task",
...
async fn apply_hard_delete(
    ...
    let (table_name, action) = match target.entity_type.as_str() {
    ...
    transaction
        .execute(
            ...
            params![target.entity_id.clone()],
        )
        .await
        .map_err(|error| SyncWorkerError::internal(format!("应用 hard delete 失败: {error}")))?;
    if target.entity_type == "task" {
        transaction
            ...
            .await
            .map_err(|error| SyncWorkerError::internal(format!("删除任务关联 TaskLink 失败: {error}")))?;
    }
...
async fn should_apply_record(
    ...
    if let Some(current_updated_at) =
        query_current_updated_at(connection, table_name, primary_key_column, primary_key).await?
    {
    ...
    if let Some((entity_type, action)) = tombstone {
        if let Some(tombstone_at) =
            query_latest_tombstone_at(connection, entity_type, primary_key, action).await?
        {
...
async fn should_apply_task_link(
    ...
    if let Some(current_updated_at) =
        query_current_updated_at(connection, "task_links", "id", link_id).await?
    {
        return Ok(current_updated_at.as_str() <= incoming_updated_at);
    }

    if let Some(tombstone_at) = query_latest_task_link_tombstone_at(connection, link_id).await? {
        return Ok(tombstone_at.as_str() < incoming_updated_at);
    }
...
async fn should_delete_task_link(
    ...
    should_apply_task_link(connection, link_id, incoming_updated_at).await
}
...
async fn query_current_updated_at(
    ...
    let mut rows = connection
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("读取当前镜像记录失败: {error}")))?;
    let row = rows
        .next()
        .await
        .map_err(|error| SyncWorkerError::internal(format!("遍历当前镜像记录失败: {error}")))?;

    row.map(|row| {
        row.get::<String>(0)
            .map_err(|error| SyncWorkerError::internal(format!("读取当前 updated_at 失败: {error}")))
    })
    .transpose()
}
...
async fn query_latest_tombstone_at(
    ...
    let mut rows = connection
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("读取 tombstone 失败: {error}")))?;
    let row = rows
        .next()
        .await
        .map_err(|error| SyncWorkerError::internal(format!("遍历 tombstone 失败: {error}")))?;

    row.map(|row| {
        row.get::<String>(0)
            .map_err(|error| SyncWorkerError::internal(format!("读取 tombstone.created_at 失败: {error}")))
    })
    .transpose()
}
...
async fn query_latest_task_link_tombstone_at(
    ...
    let mut rows = connection
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("读取 TaskLink tombstone 失败: {error}")))?;
    let row = rows
        .next()
        .await
        .map_err(|error| SyncWorkerError::internal(format!("遍历 TaskLink tombstone 失败: {error}")))?;

    row.map(|row| {
        row.get::<String>(0).map_err(|error| {
            SyncWorkerError::internal(format!("读取 TaskLink tombstone.created_at 失败: {error}"))
        })
    })
    .transpose()
}
...
async fn insert_activity_tombstone(
    ...
    let metadata = metadata
        ...
        .transpose()
        .map_err(|error| SyncWorkerError::internal(format!("序列化 tombstone metadata 失败: {error}")))?;
    transaction
        .execute(
            r#"
        ...
        .await
        .map_err(|error| SyncWorkerError::internal(format!("写入 tombstone activity 失败: {error}")))?;
    Ok(())
}
