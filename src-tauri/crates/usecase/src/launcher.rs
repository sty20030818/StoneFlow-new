...
impl<P: LauncherPorts> LauncherService<P> {
    ...
    pub async fn list_visible_spaces(&self) -> Result<Vec<LauncherSpaceSummaryDto>, UsecaseError> {
    ...
    ) -> Result<LauncherProjectsBySpaceDto, UsecaseError> {
        let space = self
            .ports
    ...
    pub async fn resolve_task_open_target(
        &self,
        task_id: &str,
    ) -> Result<LauncherResolvedOpenTarget, UsecaseError> {
