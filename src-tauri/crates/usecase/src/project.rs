...
pub trait ProjectSpaceReader: Send + Sync {
    async fn get(&self, space_id: &str) -> Result<Option<ProjectSpaceRecord>, UsecaseError>;
    async fn list_by_ids(
        &self,
        space_ids: &[String],
    ) -> Result<Vec<ProjectSpaceRecord>, UsecaseError>;
}
...
pub trait ProjectTaskCounter: Send + Sync {
    async fn count_by_project_ids(
        &self,
        project_ids: &[String],
    ) -> Result<HashMap<String, ProjectTaskCount>, UsecaseError>;
}
    ...
    T: ProjectTaskCounter,
{
    ...
    ) -> Result<Vec<ProjectOverviewItemDto>, UsecaseError> {
    ...
    ) -> Result<Vec<ProjectSidebarItemDto>, UsecaseError> {
        let scope = normalize_scope(&input.scope)?;
        let projects = self
            .persistence
    ...
    ) -> Result<ProjectDetailDto, UsecaseError> {
    ...
    pub async fn create_project(
        ...
        if self
            ...
            .is_some()
        {
            return Err(UsecaseError::conflict("当前 Space 下已存在同名 Project"));
        }
    ...
    async fn build_project_detail(
        ...
        let space = self
            ...
            .get(&project.space_id)
            .await?
            .ok_or_else(|| UsecaseError::not_found("Project 所属 Space 不存在"))?;
        let counts = self
            .task_counter
...
fn normalize_scope(input: &ProjectScopeInput) -> Result<NormalizedScope, UsecaseError> {
    match input.kind {
...
fn parse_overview_view(value: &str) -> Result<ProjectOverviewView, UsecaseError> {
    match value.trim() {
        ...
        _ => Err(UsecaseError::validation("未知的 Project Overview 视图")),
    }
}
