...
pub trait TaskLinkPersistence: Send + Sync {
    ...
    async fn get(&self, link_id: &str) -> Result<Option<TaskLinkRecord>, UsecaseError>;
    async fn list_by_task(&self, task_id: &str) -> Result<Vec<TaskLinkRecord>, UsecaseError>;
    ...
    ) -> Result<TaskLinkRecord, UsecaseError>;
    async fn update(
        &self,
        connection: &Self::Connection,
        link_id: &str,
...
pub trait TaskLinkTaskReader: Send + Sync {
    async fn get(&self, task_id: &str) -> Result<Option<TaskLinkTaskRecord>, UsecaseError>;
}
    ...
    T: TaskLinkTaskReader,
{
    ...
    ) -> Result<Vec<TaskLinkDto>, UsecaseError> {
    ...
    ) -> Result<TaskLinkDto, UsecaseError> {
        let task_id = validate_task_id_for_link(&input.task_id)?;
    ...
    pub async fn update_task_link(
        ...
        let current = self
            ...
            .get(&link_id)
            .await?
            .ok_or_else(|| UsecaseError::not_found("链接不存在"))?;
    ...
    pub async fn delete_task_link(
        ...
        if !deleted {
            return Err(UsecaseError::not_found("链接不存在"));
        }
