    ...
    P: ActivityPersistence,
{
    ...
    /// 以独立事务记录一条 Activity。
    pub async fn record_activity(&self, input: RecordActivityInput) -> Result<(), UsecaseError> {
    ...
    ) -> Result<Vec<ActivityTimelineEntry>, UsecaseError> {
        let entity_id = normalize_required_text(&input.entity_id, "Activity entity_id")?;
        let limit = input.limit.unwrap_or(50);
        if limit == 0 {
            return Err(UsecaseError::validation("Activity limit 必须大于 0"));
        }
...
fn build_activity_records(
    input: RecordActivityInput,
) -> Result<(ActivityEventRecord, Vec<ActivityChangeRecord>), UsecaseError> {
...
mod tests {
    ...
    impl ActivityPersistence for FakePersistence {
        ...
        async fn commit(&self, _connection: Self::Connection) -> Result<(), UsecaseError> {
            Ok(())
        }
