...
| **useShellNavStore**             | （已删 · 2026-07-20）   | 曾与 URL 双真相                       | **已删除**；`activeSection` / spaceId 直接读 `shellRoute`+`scope`                                                   |
...
3. **URL 真相在 Router**；分区/spaceId **直接读** `shellRoute` + `scope`（已无可写 nav store）。
