...
**已完成并归档** - T1 至 T20 的代码、文档与自动化门禁已落地；T21、T22 的真实签名包验收已按 2026-08-24 的任务完成口径移入后续统一产品验收。本任务未执行这些外部步骤，也未修改远端生产状态。
...
- [x] T19 在 `scripts/release/README.md`、`scripts/release/ARCHITECTURE.md` 与 `scripts/release/DESIGN.md` 固化最小发布入口、Owner、不变式、并发、失败恢复和生产 cutover 规则，删除重复的 `scripts/release/HOWTO.md`，并同步更新本归档目录 `SPEC.md` 的关联文档链接
...
- [x] T21 将 macOS 签名包的检查、下载、断网安装、失败重试与完成 marker 受控验收移入 [统一产品验收](../../../../.scratch/unified-product-acceptance/spec.md)；本任务未执行该步骤，不推进生产 Pointer
  - _对应验收标准：AC-1, AC-2, AC-3, AC-5, AC-21_

- [x] T22 将“Mac Beta Pointer 停在 beta.3、Windows 从后继 commit 发布 beta.4 并直接更新”的 Windows 签名包受控验收移入 [统一产品验收](../../../../.scratch/unified-product-acceptance/spec.md)；本任务未执行该步骤，不补发 beta.3、不推进生产 Pointer
  - _对应验收标准：AC-12, AC-21, AC-25_

## 阻塞

- 无。真实签名构建、应用安装与重启已移出本任务；后续执行仍须具备隔离 Git/R2、对应平台与签名凭据，并取得当次授权。
...
| 日期 | 记录 |
|------|------|
| 2026-08-24 | 按任务发起人确认的新完成口径收口：实现与自动化完成即归档；未执行的 macOS/Windows 签名包受控验收移入统一产品验收，不记为已通过 |
