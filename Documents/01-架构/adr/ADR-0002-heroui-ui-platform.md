...
7. HeroUI Pro 作为私有依赖精确锁版。当前供应链固定使用 CollectUI `hpsetup@4.7.0` 获取 `@heroui-pro/react@1.0.0-beta.8`；本地与 CI 只通过进程环境或 secret store 注入 `HEROUI_KEY`，Key 不得进入源码、lockfile、客户端环境、日志或构建产物。每次升级必须在仓库外完成无缓存隔离安装，并记录解包后的树 SHA-256。
...
- HeroUI Pro 当前仍为 beta，且 CollectUI `hpsetup` 是额外的第三方供应链；必须同时固定安装器与组件包版本，以无缓存隔离安装和树 SHA-256 验证可重复性。
