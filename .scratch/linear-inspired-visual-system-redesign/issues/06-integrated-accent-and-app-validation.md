...
- [ ] 默认钴蓝在 Shell/Sidebar、PageFrame、TaskBoard/RowShell、Command/Menu/Popover、Modal/Sheet/Detail、Settings/Form 与 Launcher 上完成全部适用状态矩阵验收，并确认 `6/8/12px` 圆角、`28/32/36px` 密度、结构边框与浮层阴影关系一致。
...
- [ ] 静态门禁确认页面不再重写 HeroUI 通用皮肤，`patterns`、旧 `base`、旧 token/shadcn adapter、Dark 与兼容 alias 已删除且没有回流。
