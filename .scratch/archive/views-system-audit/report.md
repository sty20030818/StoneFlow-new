...
2026-09-14 范围更正：本报告保留最初审计现场与判定；修复后的当前结论以[批准规格](./spec.md)、[09 验证](./09-verification.md)及[覆盖表](./09-coverage.md)为准。规格第 185／193 行要求分层记录同步证据，V01 以 SQLite／Outbox／真实投影往返验收；第 108／109／214 行允许完整定义不足时明确失败，并排除未授权的历史数据与远端协议改造。因此不将真实双设备通过、0.2.0 混写支持决策或历史云端自动修复策略追加为整个包完成前提；这些能力保留为未验证／未承诺支持／发布边界。现有功能、原始场景及真实 200% 缩放已有证据，当前仅实际错误朗读待确认，未结案归档。
