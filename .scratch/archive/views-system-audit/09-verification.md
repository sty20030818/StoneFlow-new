...
| 层面 | 本轮证据 | 能证明的范围 |
| --- | --- | --- |
| 构建与运行 | 首轮为 `fc8a9de6`；最终 `StoneFlow Views Acceptance.app` 加入 09 当前源码，[manifest](./evidence-09/build-source.json)的 8 个改动路径已与工作树核对一致；原临时日志记录 beforeBuild 通过、Rust 构建 49.22 秒、成功生成 bundle。临时日志已不在当前环境，归档保留此结果与源码 manifest | 新构建包含 sticky 修复；原生 UI 使用 `tauri://localhost`。界面仍显示版本 0.2.0，版本字符串不代替源码来源 |
| 原生交互 | 原验收保存[项目结果](./evidence-09/screenshots.md#project-all)、[分组计数](./evidence-09/screenshots.md#group-counts)、[sticky 修复前](./evidence-09/screenshots.md#sticky-before)／[修复后](./evidence-09/screenshots.md#sticky-after)、[窄窗弹窗](./evidence-09/screenshots.md#narrow-editor)五张实际 JPEG；临时 AX 文本编号 00–96 补充过程 | 实际 Tauri 主窗口的操作、可见成员、计数、URL、选中项和部分焦点；不以文件编号推断每次操作成功 |
...
| 真实 200% 缩放 | [隔离构建与恢复记录](./evidence-09/native-zoom200.json)、[长名称编辑](./evidence-09/screenshots.md#zoom200-editor)和[错误恢复界面](./evidence-09/screenshots.md#zoom200-error)两张 JPEG；只在隔离构建添加 `window.set_zoom(2.0)` 测试行，前端与 `51da8063` 产品源码一致 | 实际 WebView 2.0 缩放下代表性控件可见、键盘重试可达；不是缩小窗口模拟，也没有新增产品缩放功能或代签读屏 |
...
| 操作链 | 实际结果 | 证据 |
| --- | --- | --- |
| 项目“未完成” → 待执行 → 保存 → 新 View → 未完成 → 全部 | 未完成 6 项，保存的待执行 2 项；返回未完成恢复 6 项，全部 8 项含已完成。新 View 被选中，返回项目后 URL 无 `f`，全部仅有 `v=all` | `01-project-active.txt`–`05-project-all.txt`；[项目全部截图](./evidence-09/screenshots.md#project-all)；首轮 JSON 的 project scope/context/base 与 2 个 todo 成员检查 |
...
| 项目 337 项主／子分组与续页 | “全部”从 150 → 300 → 337 项；显示空优先级组，高／低为 0；已加载组选择 38 项，后续展开／折叠和续页完成后仍为 38，未自动选择新增成员 | `42-large-all.txt`、`43-large-two-level-first-page.txt`、`48-large-select-loaded.txt`–`53-large-exhausted.txt`；[分组总数与已选 38](./evidence-09/screenshots.md#group-counts) |
...
| sticky 修复后的新构建复测 | Saved 初页 150，End 后用 Page Down 加载到 300，再用 End 到达下一窗口，最终加载全部 337；到末行“分页 320｜已取消”时，sticky 正确显示“无优先级 › 已取消 22” | `81-fixed-saved-first-page.txt`、`84-fixed-next-page.txt`、`85-fixed-third-page.txt`、`86-fixed-last-row.txt`；[修复后截图](./evidence-09/screenshots.md#sticky-after) |
| 原生窄窗、长名称与编辑键盘 | 系统 Left 窗口操作后截图为 736×863；长名称视图仍显示正确的 1 个任务，编辑弹窗全部控件可见。Tab 明确到达关闭、取消、保存按钮；Escape 关闭后焦点回到“视图操作” | `87-native-narrow.txt`–`90-native-narrow-tab.txt`、`93-native-narrow-cancel-focus.txt`–`95-native-narrow-cancel-return.txt`；[窄窗编辑截图](./evidence-09/screenshots.md#narrow-editor)。92 的 AX 未明确标记名称框焦点，不将其列为已确认 |
...
| 真实 200% 编辑与错误恢复 | 长名称编辑的标题、名称、关闭／取消／保存均在窗口内可见；清空名称后保存禁用，Escape 取消并归焦。坏 scope 标题、详情、返回／重试均完整可见；Tab 到返回再到重试，Return 激活加载，完成后保留错误及重试焦点 | [native-zoom200.json](./evidence-09/native-zoom200.json)、[编辑截图](./evidence-09/screenshots.md#zoom200-editor)、[错误截图](./evidence-09/screenshots.md#zoom200-error)。同步未配置远端，未访问正式数据；不声称坏定义恢复成功或所有屏幕组合通过 |
...
- **sticky 尾页标题错误已修复并复测。** [修复前](./evidence-09/screenshots.md#sticky-before)在已取消尾页仍显示“等待中 22”；[useTaskBoardSticky](../../../src/features/task/hooks/useTaskBoardSticky.ts)现在以同一个 reconcile 在每次 layout commit 校正标题，同组滚动继续通过 RAF 更新相邻标题推挤。两条[真实 virtualizer 回归](../../../src/features/task/hooks/useTaskBoardSticky.test.tsx)先红后绿，包含冻结 RAF 场景；新构建[相同尾页](./evidence-09/screenshots.md#sticky-after)标题已正确。测试 fixture 直接读取未 memo 的真实 API，仅该行有带解释的 `react/incompatible-library` 免除；没有新增依赖。
