...
这不替代 Tauri WebView、真实设备同步、200% 原生缩放或屏幕阅读器朗读验收。

## 尚未闭合的兼容与验收

1. **0.2.0 与新版同时编辑未获支持。** 旧版普通编辑推进 generation，较低代际待上传编辑仍可能被忽略。已询问用户是否统一升级所有写入设备，或需要补充混用的迁移和冲突恢复；尚未收到选择，不把等待视为同意。
2. **旧云端已丢失的完整定义不会自动回传恢复。** 旧版 `create g1 → rename g2 delta` 可使远端仅余变化字段；本次本机 hydrate 不会补回云端缺失内容。新设备缺少完整定义时明确失败。自动同 ID 修复涉及迁移和并发规则，需要另行确定；没有修改用户云端数据。
3. 原生 View 实际同步与编辑验收仍未进行。本票暂不标记完成，也不以 UI Lab 或 SQLite 夹具代签。
